    ...
    public void exec(SkypeUser user, SkypeConversation group, String used, String[] args, SkypeMessage message) {
        if (args.length < 2) {
            ...
            if (show == null) {
                ...
                group.sendMessage(encode("Removed " + user.getDisplayName().orElse(user.getUsername()) + "'s progress on ") + bold(encode(show.getDisplay())));
                ...
                SuperChatController.PROGRESS.put(show.getMainName(), prg);
                group.sendMessage(encode("Set " + user.getDisplayName().orElse(user.getUsername()) + "'s progress on ") + bold(encode(show.getDisplay())) + encode(" to " + ep));
                SuperChatController.saveProgress();
            }
        }
    }

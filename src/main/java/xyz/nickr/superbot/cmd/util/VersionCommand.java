...
public class VersionCommand implements Command {
    ...
    public void exec(Sys sys, User user, Group group, String used, String[] args, Message message) {
        ...
        for (int i = 0; i < SuperBotController.GIT_COMMIT_IDS.length; i++) {
            ...
            builder.newLine().italic(true).escaped(author + " ").italic(false).escaped(msg).italic(true).escaped(" ([%s](https://github.com/nickrobson/SuperBot/commit/%s))", id.substring(0, 8), id).italic(false).build();
        }
